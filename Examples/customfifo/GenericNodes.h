...
public:
    ...
    */
    ...
    virtual int getAdditionalArg() const = 0;
...
class MyFIFO<T,length,0,0>: public FIFOBase<T> 
{
    public:
        explicit MyFIFO(T *buffer,int delay,int additional):
        mBuffer(buffer),
        readPos(0),
        writePos(delay),
        ...
        explicit MyFIFO(void *buffer,int delay,int additional):
        mBuffer((T*)buffer),
        readPos(0),
        writePos(delay),
        mAdditional(additional) {};
        ...
        /* 
        MyFIFO are fixed and not made to be copied or moved.
        */
        MyFIFO(const MyFIFO&) = delete;
        MyFIFO(MyFIFO&&) = delete;
        MyFIFO& operator=(const MyFIFO&) = delete;
        MyFIFO& operator=(MyFIFO&&) = delete;
...
/* Buffer, Synchronous */
template<typename T, int length>
class MyFIFO<T,length,1,0>: public FIFOBase<T> 
{
    public:
        /* Delay always zero when this version is generated.
           Delay value not used.
        */
        explicit MyFIFO(T *buffer,int,int additional):
        mBuffer(buffer),
        mAdditional(additional) {};
        explicit MyFIFO(void *buffer,int,int additional):
        mBuffer((T*)buffer),
        mAdditional(additional) {};
        ...
        */
        ...
        /* 
           ...
           and this version of the MyFIFO is
...
/* Real MyFIFO, Asynchronous */
template<typename T, int length>
class MyFIFO<T,length,0,1>: public FIFOBase<T> 
{
    public:
        explicit MyFIFO(T *buffer,int delay,int additional):
        mBuffer(buffer),
